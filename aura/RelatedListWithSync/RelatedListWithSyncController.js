({
    initRecords: function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (returnvalue.length === 1 && returnvalue[0]['Selected_Default__c']===false) {
                //var objectReturn = JSON.stringify(returnvalue);
                var iddone = returnvalue[0]['Id'];
                //alert(iddone);
                ...
                //alert(JSON.parse(objectReturn));
                //alert(JSON.stringify(returnvalue));

                
                /*var obj = JSON.stringify(returnvalue);
                var stringify = JSON.parse(obj);
                var idd = stringify[0]['Id'];
                alert (idd);
                for (var i = 0; i < stringify.length; i++) {
                    alert(stringify[i]['Id']);
                }*/
    ...
    onClick : function(component, event, helper) {
       var recordId = component.get('v.recordId');
        var navEvt;
        if(recordId) {
            navEvt = $A.get("e.force:navigateToRelatedList ");
            navEvt.setParams({
                "relatedListId": "Project_Costings__r",
    ...
    fetchRecords: function(component, event, helper) {
        var action = component.get("c.fetchRecordCountOppty");
        var recordId = component.get("v.recordId");
...
*/
    ...
    SaveClean: function(component, event, helper) {
        ...
        if (helper.requiredValidation(component, event)){            
            ...
            action.setParams({
                'lstOfDetails': component.get("v.CostingList"),
                'rowtosync': component.get("v.CostingList.Id"),
            });
            action.setCallback(this, function(response) {
                var state = response.getState();
                if (state === "SUCCESS") {
    ...
    newContact: function(component, event, helper) {
        ...
        createContact.setParams({
            "entityApiName": "Project_Costing__c",
            "defaultFieldValues": {
